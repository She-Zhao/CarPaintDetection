...
def compute_phase_cuda(datapath,aa):
    th1 = aa[0]
    th2 = aa[1]
    th3 = aa[2]
    th4 = aa[3]
...
def fitness(binary, start, precision,idx,photo_path,flag_generation):
    ...
    img_phase = compute_phase_cuda(photo_path,aa)
    gre = cal_gradient(img_phase[idx])
    Sss = cal_S(img_phase[idx])

    if flag_generation <= 100:
        fit = 1 / gre
    else:
        fit = 1 / (1 + math.exp(gre))
        if flag_generation % 20 == 0:
            fit = fit / 2 ** Sss
...
def final_cal(photo_path):
    ...
    best = genetic_cal(30, 300, 0.7, 0.05, 0.1, 5, 1, idx, photo_path)
    dict_key = photo_path[-3:]
    dict[dict_key] = best

    save_img = compute_phase_cuda(photo_path,best)
    save_file = r'.\pic'
    if not os.path.exists(save_file):
        os.makedirs(save_file)
    save_path = os.path.join(save_file, photo_path[-3:]) + '.png'
    cv.imwrite(save_path, save_img[idx])
...
if __name__ == "__main__":
    ...
    begin = time.time()
    #点位照片路径###########################################
    datapath = r'D:\datapath\20240321\yx'              ###
    ######################################################
    files = os.listdir(datapath)
    with ThreadPoolExecutor(max_workers=5) as executor:  # max_workers指定每次最多处理几组照片
        executor.map(final_cal, [os.path.join(datapath, file) for file in files])
    with open(".\output.csv", "w", newline="") as file:
        writer = csv.writer(file)
        for d_key, d_value in dict.items():
            writer.writerow([d_key, d_value])
    end = time.time()
    t = end - begin
    print("总时间：", t)
