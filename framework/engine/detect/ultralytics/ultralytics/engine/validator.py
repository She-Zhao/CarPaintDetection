...
class BaseValidator:
    ...
    def __call__(self, trainer=None, model=None):
        ...
        if self.training:
            ...
            model.eval()
            model.warmup(imgsz=(1 if pt else self.args.batch, self.data.get("nimg", self.data["channels"]), imgsz, imgsz))  # warmup
        ...
        for batch_i, batch in enumerate(bar):
            ...
            visualize = self.args.project + "/" + self.args.name + "/" + batch["im_file"][0].split('/')[-1].split('.')[0] if self.args.visualize else False
            if visualize:
            ...
            # Inference
            with dt[1]:
                preds = model(batch["img"], augment=augment, visualize=visualize)
