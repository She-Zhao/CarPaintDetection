...
**注意：当前这套代码实现了对成像、检测单元的实现与控制，机械臂控制单元目前尚未接入。对于机械臂单元的集成，已在[系统架构设计](docs/architecture.md)中的第六点进行了说明。后续同学调试的过程中，先跑起来本套代码的流程（包括软硬件），然后再集成机械臂。**
...
| 组件 | 型号/规格 | 数量 | 用途 |
|------|-----------|------|------|
| **主机** | 联想小新Mini (Windows 11) | 1台 | 投射图像与、UI交互、连接机械臂 |
| **从机** | NVIDIA Jetson Orin | 1台 | 实时图像处理与算法执行 |
| **工业相机** | Basler a2A2448-105g5mBAS | 2台 | 高精度双相机图像采集 |
