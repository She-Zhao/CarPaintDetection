...
```bash
# 从机端
python framework/core/client.py   # 原slave.py

# 主机端
python framework/core/server.py   # 原host.py

# 机械臂
python framework/core/robot.py    # 完全未改动
```
...
- [ ] `engine/detect/detect_api.py`             - 支持参数配置、模型选择         @赵射
   ...
   - [ ] `engine/detect/detect_api.py`          - 新检测模型算法代码及权重, 自建一个类       @刘佳璇
   - [ ] `engine/detect/detect_api.py`          - 还需要改一下__call__方法，因为不同模型的输入图像不一样，需要跟@刘佳璇对一下新模型的输入形式，最好直接将输入包装成[N, C, H, W]的Tensor
