...
class RandomBrightness:
    """
    Randomly adjust the brightness of grayscale images.

    This class applies random brightness augmentation to grayscale images within predefined limits.
    For single-channel images, it applies random brightness adjustment directly.
    For multi-channel images, it applies independent random brightness adjustment to the first channel
    and the same random brightness adjustment to all other channels.

    Attributes:
        bgain (float): Maximum variation for brightness. Range is typically [0, 1].

    Methods:
        __call__: Apply random brightness augmentation to an image.

    Examples:
        >>> import numpy as np
        >>> from ultralytics.data.augment import RandomBrightness
        >>> augmenter = RandomBrightness(bgain=0.3)
    ...
    def __init__(self, bgain=0.3) -> None:
        """
        Initialize the RandomBrightness object for random brightness augmentation.

        This class applies random adjustments to the brightness of grayscale images within specified limits.

        Args:
            bgain (float): Maximum variation for brightness. Should be in the range [0, 1].

        Examples:
            >>> brightness_aug = RandomBrightness(bgain=0.3)
            >>> brightness_aug(labels)
        """
        self.bgain = bgain

    def __call__(self, labels):
        """
        Apply random brightness augmentation to an image within predefined limits.

        This method modifies the input image by randomly adjusting its brightness.
        For single-channel images, applies random brightness adjustment directly.
        For multi-channel images, applies independent random brightness adjustment to the first channel
        and the same random brightness adjustment to all other channels.
        ...
        Returns:
            (None): The function modifies the input 'labels' dictionary in-place, updating the 'img' key
                with the brightness-augmented image.

        Examples:
            >>> brightness_augmenter = RandomBrightness(bgain=0.3)
            >>> # Single channel
            >>> labels = {"img": np.random.randint(0, 255, (100, 100), dtype=np.uint8)}
            >>> brightness_augmenter(labels)
            >>> # Multi-channel
            >>> labels = {"img": np.random.randint(0, 255, (100, 100, 4), dtype=np.uint8)}
            >>> brightness_augmenter(labels)
        """
        img = labels["img"]
        if self.bgain == 0:
        ...
        if len(img.shape) == 2:  # Single channel image
            ...
            r = np.random.uniform(-1, 1) * self.bgain
            lut = np.clip(x * (r + 1), 0, 255).astype(dtype)
            labels["img"] = cv2.LUT(img, lut)
            
        elif len(img.shape) == 3:  # Multi-channel image
            channels = img.shape[2]
            if channels == 1:  # Single channel in 3D array
                r = np.random.uniform(-1, 1) * self.bgain
                lut = np.clip(x * (r + 1), 0, 255).astype(dtype)
                labels["img"] = cv2.LUT(img, lut)
            else:  # Multi-channel image
                # Generate random brightness adjustment for first channel
                r1 = np.random.uniform(-1, 1) * self.bgain
                lut1 = np.clip(x * (r1 + 1), 0, 255).astype(dtype)
                
                # Generate same random brightness adjustment for other channels
                r2 = np.random.uniform(-1, 1) * self.bgain
                lut2 = np.clip(x * (r2 + 1), 0, 255).astype(dtype)
                
                # Apply different LUT to first channel and same LUT to other channels
                img[:, :, 0] = cv2.LUT(img[:, :, 0], lut1)
                if labels.get("nimg", None) is not None:
                    if channels == labels["nimg"]+1:
                        channels = channels - 1
                        img[:, :, -1] = cv2.LUT(img[:, :, -1], lut1)
                # for i in range(1, channels):
                #     img[:, :, i] = cv2.LUT(img[:, :, i], lut2)
                labels["img"] = img
        return labels
...
def v8_transforms(dataset, imgsz, hyp, stretch=False):
    ...
    return Compose(
        [
            ...
            RandomBrightness(bgain=hyp.bgain),
