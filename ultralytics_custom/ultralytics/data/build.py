...
from ultralytics.data.dataset import GroundingDataset, YOLODataset, YOLOMultiModalDataset, YOLOMultiIMGDataset
from ultralytics.data.loaders import (
    LOADERS,
    LoadImagesAndVideos,
    LoadMultiImagesfromTxt,
    LoadPilAndNumpy,
    LoadScreenshots,
    LoadStreams,
    LoadTensor,
...
def build_yolo_dataset(cfg, img_path, batch, data, mode="train", rect=False, stride=32, multi_modal=False):
    ...
    print(f"###############################################build_yolo_dataset {dataset}")
    return dataset(
        img_path=img_path,
        imgsz=cfg.imgsz,
        batch_size=batch,
...
def load_inference_source(source=None, batch: int = 1, vid_stride: int = 1, buffer: bool = False, channels: int = 3):
    ...
    if tensor:
        ...
        try:
            ...
            # 如果LoadImagesAndVideos失败，尝试使用LoadMultiImagesfromTxt
            dataset = LoadMultiImagesfromTxt(source, batch=batch, channels=channels)
            print(f"###############################################LoadMultiImagesfromTxt {source}")
