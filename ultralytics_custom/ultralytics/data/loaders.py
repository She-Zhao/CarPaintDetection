...
class LoadMultiImagesfromTxt:
    """
    A class for loading and processing images from txt file for YOLO object detection.

    This class manages the loading and pre-processing of image data from txt file.
    ...
    def __init__(self, path: Union[str, Path, List], batch: int = 1, channels: int = 3, imgmode: list = IMAGE_MODE):
        """
        Initialize dataloader for images and videos, supporting various input formats.
        ...
        for p in sorted(path) if isinstance(path, (list, tuple)) else [path]:
            ...
            #######################################################################
            ########################  Modified code  ##############################
            a = str(parent / Path(re.sub(r"(/images/)", rf"\1{imgmode[0]}/", p)))
            if os.path.isfile(a):
        ...
        while len(imgs) < self.bs:
            ...
            if path.rpartition(".")[-1].lower() == "heic":
                ...
                for m in self.imgmode:
                    ...
                    imt = imread(f, flags=self.cv2_flag)
                    imgset.append(imt.squeeze())
                im0 = np.ascontiguousarray(np.array(imgset).transpose(1, 2, 0))
                #######################################################################
