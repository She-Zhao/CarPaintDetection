...
IMAGE_MODE = ['AP', 'sin0', 'sin1', 'sin2', 'sin3'] # 'GC0', 'GC1', 'GC2', 'GC3', 'GC4',            # modifided

def verify_multi_image_label(args):
    """Verify one image-label pair."""
    im_file, lb_file, prefix, keypoint, num_cls, nkpt, ndim, single_cls = args
    file_name = im_file                                                                             # modifided
    # Number (missing, found, empty, corrupt), message, segments, keypoints
    nm, nf, ne, nc, msg, segments, keypoints = 0, 0, 0, 0, "", [], None
    try:
        # Verify images
        for m in IMAGE_MODE:                                                                        # modifided
            im_file = re.sub(r"(/images/)", rf"\1{m}/", file_name)                                  # modifided
...
class YOLOMultiIMGDataset(YOLODataset):
    """
    ...
    def __init__(self, *args, data=None, task="detect", **kwargs):
        ...
        self.use_fusion_label = self.data.get('use_fusion_label', False)
    ...
    def get_img_files(self, img_path: Union[str, List[str]]) -> List[str]:
        ...
        ##############################  Modified code  #########################################
        check_file_speeds([re.sub(r"(/images/)", rf"\1{IMAGE_MODE[0]}/", x) for x in im_files]
                          , prefix=self.prefix)  # check image read speeds
        ########################################################################################
        return im_files
    ...
    def cache_labels(self, path=Path("./labels.cache")):
        ...
        with ThreadPool(NUM_THREADS) as pool:
            results = pool.imap(
                func=verify_multi_image_label,                                                  # modifided
                iterable=zip(
                    self.im_files,
                    self.label_files,
                    repeat(self.prefix),
    ...
    def load_image(self, i, rect_mode=True):
        ...
        if im is None:  # not cached in RAM
            if fn.exists():  # load npy
                ...
                ft = f
                for m in IMAGE_MODE:                                                           # modifided
                    f = re.sub(r"(/images/)", rf"\1{m}/", ft)                                  # modifided
                    imt = cv2.imread(f, flags=self.cv2_flag)  # BGR  # grayscale
                    imgs.append(imt)
                im = np.array(imgs).transpose(1, 2, 0)
                # End -------------------------------------------------------------------------------------- #
                # Previous --------------------------------------------------------------------------------- #
                # im = cv2.imread(f)  # BGR
                # End -------------------------------------------------------------------------------------- #
    ...
    def load_fusion_labels(self, imgfile, folder_name='fusion_labels', rect_mode=True):
        fusion_path = imgfile.replace('/images/', f'/images/{folder_name}/')
        if os.path.exists(fusion_path):
            im = cv2.imread(fusion_path, cv2.IMREAD_GRAYSCALE)
    ...
    def get_image_and_label(self, index: int) -> Dict[str, Any]:
        ...
        if self.use_fusion_label and self.data.get("nimg", False):
            label["fusionlabel"] = self.load_fusion_labels(label["im_file"])
            label["nimg"] = self.data.get("nimg")
            # 拼接fusionlabel到img最后一个通道
            if label.get("img") is not None and label.get("fusionlabel") is not None:
                label["img"] = np.concatenate([label.pop("img"), label.pop("fusionlabel")], axis=-1)
    ...
    def collate_fn(batch: List[Dict]) -> Dict:
        ...
        for i, k in enumerate(keys):
            value = values[i]
            if k in {"img", "text_feats"}:
                nimg = batch[0].get("nimg", None)
                if nimg is None or value[0].shape[0] == nimg:
                    new_batch["fusion_label"] = None
                    value = torch.stack(value, 0)
                elif value[0].shape[0] == nimg+1:
                    new_batch["fusion_label"] = torch.stack([v[-1:] for v in value], 0)
                    value = torch.stack([v[:-1] for v in value], 0)
            elif k == "visuals":
                value = torch.nn.utils.rnn.pad_sequence(value, batch_first=True)
            if k in {"masks", "keypoints", "bboxes", "cls", "segments", "obb"}:
                value = torch.cat(value, 0)
