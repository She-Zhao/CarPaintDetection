...
class DetectionValidator(BaseValidator):
    ...
    def preprocess(self, batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if batch.get("fusion_label", None) is not None:
            batch["fusion_label"] = batch["fusion_label"].to(self.device, non_blocking=True)
            batch["fusion_label"] = (batch["fusion_label"].half() if self.args.half else batch["fusion_label"].float()) / 255
    ...
    def postprocess(self, preds: torch.Tensor) -> List[Dict[str, torch.Tensor]]:
        ...
        """
        if isinstance(preds[0], list) and len(preds[0]) == 3:
            preds = preds[1]
        elif preds[0].shape[1] == 1:  # preds[0] is fusion_img
            preds = preds[1]
