...
class BaseModel(torch.nn.Module):
    ...
    def _predict_once(self, x, profile=False, visualize=False, embed=None):
        ...
        Returns:
            (torch.Tensor): The last output of the model.
        """
        backbone2_len = len(self.yaml.get("backbone2", []))
        fusion_len = len(self.yaml.get("fusion", []))
        model_list_len = len(self.yaml.get("model_list", []))
        if backbone2_len and fusion_len:
            if hasattr(self, "args") and hasattr(self.args, "trainfusionmodel") and self.args.trainfusionmodel:
                x = self.predict_fusion_bkb2_trainfusionmodel(x, profile, visualize, embed)
            else:
                x = self.predict_fusion_bkb2(x, profile, visualize, embed)
            return x
        elif model_list_len:
            x = self.predict_fusion(x, profile, visualize, embed)
            return x
        # 原YOLO predict
    ...
    def predict_fusion_bkb2_trainfusionmodel(self, x, profile=False, visualize=False, embed=None):
        y, dt, embeddings = [], [], []  # outputs
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        ...
        # if ch == 1:
        #     x = x[:,0:1,:,:] # 调试用
        x, x2 = (x[:, 0:1, :, :], [x[..., i:i+1, :, :] for i in range(1, x.shape[-3])]) if backbone2_len > 0 else (x, None)     # 多输入双分支输入处理
        for m in self.model:
            if m.i == backbone_len and x2 is not None and backbone2_len:    # 到backbone2的输入层，切换输入图像
                x = x2
            ...
            # run
            if m.i >= backbone_len and m.i < backbone_len + backbone2_len:  # backbone2输入应为(b,1,h,w)，多源图像第二分支输入为[(b,1,h,w)*(n-1)]，需要分别输入处理
                # x = x
                # x = m(x)
                x = [m(x[i]) for i in range(len(x))]
            elif m.i >= backbone_len + backbone2_len and m.i < backbone_len + backbone2_len + fusion_len:   # 融合部分需要融合bkb1的输出(b,c,h,w)以及bkb2的输出[(b,c,h,w)*(n-1)]，融合部分网络的输入应为[(b,c,h,w)*n]
                # x = x
                # x = m(x)
                x = m([x[0]] + x[1])
                if isinstance(x, list) and len(x) == 2:
                    spatial_attns.append(x[1])
                    x = x[0]
                else:
                    print("fusion output is not a list of length 2")
            else:
                x = m(x)
            # print(m)
            ...
            if m.i in embed:
                ...
                if m.i == max_idx:
                    return torch.unbind(torch.cat(embeddings, 1), dim=0)
        return [spatial_attns, x]
    
    def predict_fusion_bkb2(self, x, profile=False, visualize=False, embed=None):
        import cv2
        y, dt, embeddings = [], [], []  # outputs
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        ...
        for m in self.model:
            ...
            if m.i >= backbone_len and m.i < backbone_len + backbone2_len:  # backbone2输入应为(b,1,h,w)，多源图像第二分支输入为[(b,1,h,w)*(n-1)]，需要分别输入处理
                ...
                if isinstance(x, list):
                    if visualize:
                        spatial_attn = x[1][0]
                        selected_stripe = x[1][1]
                        cv2.imwrite(visualize / f"stage{m.i}_spatial_attn.png", np.int_(((spatial_attn-torch.min(spatial_attn))/(torch.max(spatial_attn)-torch.min(spatial_attn))).cpu().numpy()[0,0]*255))
                        
                        import matplotlib.pyplot as plt
                        import math
                        f = visualize / f"stage{m.i}_selected_stripe_features.png"  # filename
                        blocks = torch.chunk(selected_stripe[0].cpu(), selected_stripe.shape[1], dim=0)  # select batch index 0, block by channels
                        n = min(32, selected_stripe.shape[1])  # number of plots
                        ...
                        LOGGER.info(f"Saving {f}... ({n}/{selected_stripe.shape[1]})")
                        plt.savefig(f, dpi=300, bbox_inches="tight")
                        plt.close()
                        
                        
                        # import cv2
                        # for k in [98,102]:#15, 25, 26
                        #     f_top = visualize / f"stage{m.i}_selected_stripe_feature{k}.png"
                        #     top_f_np = blocks[k].squeeze().numpy()
                        #     top_f_np = cv2.normalize(top_f_np, None, 0, 255, cv2.NORM_MINMAX, dtype=cv2.CV_8U)
                        #     cv2.imwrite(f_top, top_f_np)
    ...
    def predict_fusion(self, x, profile=False, visualize=False, embed=None):
        y, dt, embeddings = [], [], []  # outputs
        embed = frozenset(embed) if embed is not None else {-1}
        max_idx = max(embed)
        # model_list: ['fusion_encoder1', 'fusion_encoder2', 'fusion', 'fusion_decoder', 'backbone', 'head']
        
        fusion_encoder1_len = len(self.yaml["fusion_encoder1"])
        fusion_encoder2_len = len(self.yaml["fusion_encoder2"])
        fusion_len = len(self.yaml["fusion"])
        output_idx1 = sum([len(self.yaml[k]) for k in self.yaml["model_list"] if "fusion" in k]) - 1
        output_idx2 = sum([len(self.yaml[k]) for k in self.yaml["model_list"]]) - 1
        output = []
        # ch = self.yaml.get("ch", 3)
        # if ch == 1:
        #     x = x[:,0:1,:,:] # 调试用
        x, x2 = (x[:, 0:1, :, :], [x[..., i:i+1, :, :] for i in range(1, x.shape[-3])])     # 多输入双分支输入处理
        import cv2
        for m in self.model:
            # 到fusion_encoder2的输入层，切换输入图像
            if m.i == fusion_encoder1_len and x2 is not None and fusion_encoder2_len:    
                x = x2
            ...
            # run
            # fusion_encoder2输入应为(b,1,h,w)，多源图像第二分支输入为[(b,1,h,w)*(n-1)]，需要分别输入处理
            if m.i >= fusion_encoder1_len and m.i < fusion_encoder1_len + fusion_encoder2_len:
                # x = x
                # x = m(x)
                x = [m(x[i]) for i in range(len(x))]
            # 融合部分需要融合encoder1的输出(b,c,h,w)以及encoder2的输出[(b,c,h,w)*(n-1)]，融合部分网络的输入应为[(b,c,h,w)*n]
            elif m.i >= fusion_encoder1_len + fusion_encoder2_len and m.i < fusion_encoder1_len + fusion_encoder2_len + fusion_len:   
                # x = x
                # x = m(x)
                # x = m([x[0]] + x[1])
                if isinstance(x, list):
    ...
    def load(self, weights, verbose=True):
        ...
        csd = self.align_state_dict(csd)
        updated_csd = intersect_dicts(csd, self.state_dict())  # intersect
        self.load_state_dict(updated_csd, strict=False)  # load
        len_updated_csd = len(updated_csd)
        # 这里是yolo对输入层通道数发生变化时预训练参数的适配
        ...
        #         len_updated_csd += 1
        # 对所有除检测头外的卷积，key相同通道数不同的情况下，预训练参数对模型进行适配，包括输入层
        # 包含"yolo对输入层通道数发生变化时预训练参数的适配"
        len_updated_csd = self.align_mismatched_weights(csd, updated_csd, len_updated_csd)
        # 两个backbone分别加载预训练参数
        # self.load_backbone_weights(self, csd, updated_csd, len_updated_csd)
        if verbose:
    ...
    def align_state_dict(self, csd):
        """
        Align the state dictionary of the model with the state dictionary of the checkpoint.
        """# 将预训练模型与双分支模型的参数进行适配，以最大限度利用预训练模型
        if next(reversed(self.state_dict().keys())) == next(reversed(csd.keys())):
            return csd
        len_yolo11bkb = 11
        len_yolo11head = 13
        len_backbone = len(self.yaml["backbone"])
        len_backbone2 = len(self.yaml.get("backbone2", []))
        len_fusion = len(self.yaml.get("fusion", []))
        len_head = len(self.yaml["head"])
        len_model_list = len(self.yaml.get("model_list", []))
        
        # 创建新的OrderedDict来存储适配后的权重
        aligned_dict = OrderedDict()
        
        if len_backbone2 != 0:
            # 计算目标模型各部分的起始和结束索引
            bkb1_start, bkb1_end = 0, len_backbone - 1
            bkb2_start, bkb2_end = len_backbone, len_backbone + len_backbone2 - 1
            head_start = len_backbone + len_backbone2 + len_fusion
            head_end = head_start + len_head - 1
            
            # 1. backbone1映射：当len_yolo11bkb == len_backbone时
            if len_yolo11bkb == len_backbone:
                print("len_yolo11bkb == len_backbone")
                for key, value in csd.items():
                    if key.startswith(f"model.") and key.split(".")[1].isdigit():
                        layer_idx = int(key.split(".")[1])
                        if 0 <= layer_idx <= len_yolo11bkb - 1:  # model.0.x - model.10.x
                            # 保持相同的索引映射
                            aligned_dict[key] = value
            
            # 2. backbone2映射：当len_yolo11bkb == len_backbone2时
            if len_yolo11bkb == len_backbone2:
                print("len_yolo11bkb == len_backbone2")
                for key, value in csd.items():
                    if key.startswith(f"model.") and key.split(".")[1].isdigit():
                        layer_idx = int(key.split(".")[1])
                        if 0 <= layer_idx <= len_yolo11bkb - 1:  # model.0.x - model.10.x
                            # 映射到backbone2位置：model.11.x - model.21.x
                            new_layer_idx = bkb2_start + layer_idx
                            new_key = key.replace(f"model.{layer_idx}.", f"model.{new_layer_idx}.")
                            aligned_dict[new_key] = value
            
            # 3. head映射：当len_yolo11head == len_head时
            if len_yolo11head == len_head:
                print("len_yolo11head == len_head")
                for key, value in csd.items():
                    if key.startswith(f"model.") and key.split(".")[1].isdigit():
                        layer_idx = int(key.split(".")[1])
                        if len_yolo11bkb <= layer_idx <= len_yolo11bkb + len_yolo11head - 1:  # model.11.x - model.23.x
                            # 映射到目标head位置
                            relative_idx = layer_idx - len_yolo11bkb  # 0-12
                            new_layer_idx = head_start + relative_idx
                            new_key = key.replace(f"model.{layer_idx}.", f"model.{new_layer_idx}.")
                            aligned_dict[new_key] = value
        elif len_model_list != 0:
            idx_start, idx_end = 0, 0
            bkb_start, bkb_end, head_start, head_end = 0, 0, 0, 0
            # 计算目标模型各部分的起始和结束索引
            for l in self.yaml.get("model_list"):
                idx_end = idx_start + len(self.yaml[l]) - 1
                if l == "backbone":
                    bkb_start, bkb_end = idx_start, idx_end
                elif l == "head":
                    head_start, head_end = idx_start, idx_end
                idx_start = idx_end + 1
            ...
            for key, value in csd.items():
                if key.startswith(f"model.") and key.split(".")[1].isdigit():
                    ...
                    if 0 <= layer_idx <= len_yolo11bkb - 1:  # model.0.x - model.10.x
                        # 映射到backbone2位置：model.11.x - model.21.x
                        new_layer_idx = bkb_start + layer_idx
                        new_key = key.replace(f"model.{layer_idx}.", f"model.{new_layer_idx}.")
                        aligned_dict[new_key] = value
                    if len_yolo11bkb <= layer_idx <= len_yolo11bkb + len_yolo11head - 1:  # model.11.x - model.23.x
                        # 映射到目标head位置
    ...
    def align_mismatched_weights(self, csd, updated_csd, len_updated_csd):
        mismatched_weights = sorted(set(self.state_dict().keys()) - set(updated_csd.keys()))
        
        if len(self.yaml.get("backbone2", [])):
            len_model = len(self.yaml["backbone"]) + len(self.yaml.get("backbone2", [])) \
                    + len(self.yaml.get("fusion", [])) + len(self.yaml["head"])
            len_backbone = len(self.yaml.get("backbone", [])) + len(self.yaml.get("backbone2", []))
            
            for k in mismatched_weights:
                if int(k.split('.')[1]) >= len_backbone:
                    continue
                c1, c2, h, w = self.state_dict()[k].shape
                cc1, cc2, ch, cw = csd[k].shape
                if ch == h and cw == w:
                    c1, c2 = min(c1, cc1), min(c2, cc2)
                    self.state_dict()[k][:c1, :c2] = csd[k][:c1, :c2]
                    len_updated_csd += 1
        
        elif len(self.yaml.get("model_list", [])):
            idx_start, idx_end = 0, 0
            bkb_start, bkb_end, head_start, head_end = 0, 0, 0, 0
            # 计算目标模型各部分的起始和结束索引
            for l in self.yaml.get("model_list"):
            ...
            for k in mismatched_weights:
                idx = int(k.split('.')[1])
                if k.split('.')[2] != 'conv' or idx < bkb_start or idx > bkb_end:
                    continue
                c1, c2, h, w = self.state_dict()[k].shape
                cc1, cc2, ch, cw = csd[k].shape
                if ch == h and cw == w:
...
class DetectionModel(BaseModel):
    ...
    def __init__(self, cfg="yolo11n.yaml", ch=3, nc=None, verbose=True):
        ...
        # Define model
        ch = self.yaml["channels"] = self.yaml.get("ch", ch)  # save channels
        ...
        if isinstance(m, Detect):  # includes all Detect subclasses like Segment, Pose, OBB, YOLOEDetect, YOLOESegment
            ...
            img_ch = self.yaml.get("nimg", ch)
            m.stride = torch.tensor([s / x.shape[-2] for x in _forward(torch.zeros(1, img_ch, s, s))[ -1 if self.yaml.get("model_list", False) else slice(0, None) ]])  # forward
    ...
    def init_criterion(self):
        ...
        # if hasattr(self.args, "trainfusionmodel") and self.args.trainfusionmodel:
        #     loss = DefectAwareFusionLoss(self)
        # else:
...
def parse_model(d, ch, verbose=True):
    ...
    if "model_list" in d:
        model_list = []
        for key in d["model_list"]:
            model_list.extend(d.get(key, []))
    elif "backbone2" in d:
        if "fusion" in d:
            model_list = d["backbone"] + d["backbone2"] + d["fusion"] + d["head"]
        else:
            model_list = d["backbone"] + d["backbone2"] + d["head"]
    else:
        model_list = d["backbone"] + d["head"]
    for i, (f, n, m, args) in enumerate(model_list):  # from, number, module, args
        m = (
        ...
        if m in base_modules:
        ...
        elif m is MultiConcat:
            c2 = ch[f[0]] * d['nimg']   # bkb2有nimg-1个输入，因此concat的输出通道不只是bkb1和bkb2输出通道的简单相加，和输入图片个数有关
        elif m in frozenset(
            ...
            if c2 == -1:
                c2 = c1
            args = [c1, c2, *args[1:]]
        elif m in frozenset({DEA, DefectAwareFusionModule, DefectAwareFusionModule_old, DualStageFeatureFusion}):
            c2 = ch[f[0]]
            c1 = c2 * d['nimg']
            args = [c1, c2, *args[1:]]
        else:
        ...
        if i == 0:
            ch = []
        if i == len(d["backbone"]) and len(d.get("backbone2", [])):
            ch.pop()
        if i == len(d.get("fusion_encoder1", [])) and len(d.get("fusion_encoder2", [])):
            ch.pop()
        ch.append(c2)
        if i == len(d["backbone"]) - 1 and len(d.get("backbone2", [])):
            ch.append(ch0)
        if i == len(d.get("fusion_encoder1", []))-1 and len(d.get("fusion_encoder2", [])):
            ch.append(ch0)
