...
def feature_visualization(x, module_type: str, stage: int, n: int = 32, save_dir: Path = Path("runs/detect/exp")):
    ...
    if stage not in {4,6,9,10,15,17,21,22,23,24}:
        return
    ...
    if isinstance(x, torch.Tensor):
        ...
        if height > 1 and width > 1:
            ...
            # import cv2
            # # 额外保存激活总和最高的5个通道
            # channel_sums = blocks.sum(dim=(1, 2))  # [channels] - 每个通道的激活总和
            # top_k = min(5, channels)  # 最多5个通道
            # top_values, top_indices = torch.topk(channel_sums, k=top_k, dim=0)
            # top_indices = top_indices.tolist()
            
            # f_top = save_dir / f"stage{stage}_{module_type.rsplit('.', 1)[-1]}_top5_features.png"
            # n_top = len(top_indices)
            # _, ax_top = plt.subplots(math.ceil(n_top / 8), 8, tight_layout=True)
            # ax_top = ax_top.ravel()
            # plt.subplots_adjust(wspace=0.05, hspace=0.05)
            # for idx, channel_idx in enumerate(top_indices):
            #     ax_top[idx].imshow(blocks[channel_idx].squeeze())
            #     ax_top[idx].set_title(f'Ch{channel_idx}\nSum:{top_values[idx].item():.2f}', fontsize=6)
            #     ax_top[idx].axis("off")
            # for idx in range(n_top, len(ax_top)):
            #     ax_top[idx].axis("off")
            
            # LOGGER.info(f"Saving {f_top}... (top {n_top}/{channels} channels by activation sum)")
            # plt.savefig(f_top, dpi=300, bbox_inches="tight")
            # plt.close()
    elif isinstance(x, list) and len(x) == 4:
        for i, x_ in enumerate(x):
            if not isinstance(x_, torch.Tensor):
                break
            _, channels, height, width = x_.shape  # batch, channels, height, width
            if height > 1 and width > 1:
                f = save_dir / f"stage{stage}_f{i}_{module_type.rsplit('.', 1)[-1]}_features.png"  # filename

                blocks = torch.chunk(x_[0].cpu(), channels, dim=0)  # select batch index 0, block by channels
                n = min(n, channels)  # number of plots
                _, ax = plt.subplots(math.ceil(n / 8), 8, tight_layout=True)  # 8 rows x_ n/8 cols
                ax = ax.ravel()
                plt.subplots_adjust(wspace=0.05, hspace=0.05)
                for i in range(n):
                    ax[i].imshow(blocks[i].squeeze())  # cmap='gray'
                ...
                plt.savefig(f, dpi=300, bbox_inches="tight")
                plt.close()
                # np.save(str(f.with_suffix(".npy")), x_[0].cpu().numpy())  # npy save
